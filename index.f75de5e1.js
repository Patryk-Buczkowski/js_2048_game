    ...
    class Board {
        ...
        appendOneBox() {
            ...
            if (this.occupiedPlaces().length < 16) {
                ...
                while(occupiedPlaces.some((cell)=>cell.y === xRandomIndex && cell.x === yRandomIndex)){
        ...
        availableMoves() {
            const move = this.occupiedPlaces().some(({ y, x })=>{
                return this.getBoardField(y, x - 1) === this.getBoardField(y, x) || this.getBoardField(y, x + 1) === this.getBoardField(y, x) || this.getBoardField(y + 1, x) === this.getBoardField(y, x) || this.getBoardField(y - 1, x) === this.getBoardField(y, x);
            });
            return move;
        }
        getBoardField(y, x) {
        ...
        check2048() {
            if (this.board.flat().some((elem)=>elem === 2048)) messageWin.classList.toggle("hidden");
        }
        ...
        occupiedPlaces() {
            ...
            for(let y = 0; y < boardSize; y++){
                for(let x = 0; x < this.board[y].length; x++)if (this.board[y][x] !== 0) occupiedPlaces.push({
                    "y": y,
                    "x": x
                });
            }
        ...
        render() {
            for(let y = 0; y < boardSize; y++)for(let x = 0; x < this.board[y].length; x++){
